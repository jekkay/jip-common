...
public class RedBlackTree<T extends Comparable<T>> {
    ...
    public List<T> all() {
        ...
        return collectChildren(result, this.root);
    }

    private List<T> collectChildren(List<T> result, RedBlackNode<T> node) {
        if (isNil(node)) {
            return result;
        }
        // 前序遍历收集
        collectChildren(result, node.left);
        if (node.key != null) {
            result.add(node.key);
        }
        collectChildren(result, node.right);
        return result;
    }
    ...
    private void destroy(RedBlackNode<T> node) {
        if (isNil(node)) {
            return;
        }
        destroy(node.left);
    ...
    public String toString() {
        return String.format("tree size %s", size());
    }

    public void fixMinMaxValue() {
        if (!minMaxValid) {
            reCalculateMinMax();
        }
    }
    ...
    public void setMinMaxInvalid() {
        if (isMinMaxValid()) {
            minMaxValid = false;
            doSetMinMaxInvalid(getRoot());
        }
    }

    private void doSetMinMaxInvalid(RedBlackNode<T> node) {
        if (isNil(node)) {
            return;
        }
        doSetMinMaxInvalid(node.getLeft());
        doSetMinMaxInvalid(node.getRight());
        node.minValue = node.maxValue = null;
    }

    public void reCalculateMinMax() {
        doReCalculateMinMax(getRoot());
        minMaxValid = true;
    }

    private void doReCalculateMinMax(RedBlackNode<T> node) {
        if (isNil(node)) {
            return;
        }
        doReCalculateMinMax(node.getLeft());
        doReCalculateMinMax(node.getRight());
        node.minValue = minValue(node.getLeft() != null ? node.getLeft().minValue : null,
                node.getRight() != null ? node.getRight().minValue : null,
                node.getKey());
        node.maxValue = maxValue(node.getLeft() != null ? node.getLeft().maxValue : null,
                node.getRight() != null ? node.getRight().maxValue : null,
                node.getKey());
    }

    // 需要重写下
    protected T minValue(T value) {
        return value;
    }

    protected T minValue(T value1, T value2) {
        value1 = minValue(value1);
        value2 = minValue(value2);
        if (value1 == null) {
            return value2;
        }
        if (value2 == null) {
    ...
    protected T minValue(T value1, T value2, T value3) {
        return minValue(minValue(value1, value2), value3);
    }

    // 需要重写下
    protected T maxValue(T value) {
        return value;
    }

    protected T maxValue(T value1, T value2) {
        value1 = maxValue(value1);
        value2 = maxValue(value2);
        if (value1 == null) {
            return value2;
        }
        if (value2 == null) {
    ...
    protected T maxValue(T value1, T value2, T value3) {
        return maxValue(maxValue(value1, value2), value3);
    }
